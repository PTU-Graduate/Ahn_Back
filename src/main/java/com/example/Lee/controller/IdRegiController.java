...
public class IdRegiController {
	...
	public IdRegiController(IdRegiService idRegiService) {
		this.idRegiService = idRegiService; // 생성자를 통해 주입받은 서비스 객체를 필드에 할당
	}

	@PostMapping("/PTU/IDregister") // "/PTU/register" 경로로 POST 요청을 받음
	public ResponseEntity<CommonResponseModel> idRegister(@RequestBody Map<String, String> requestData) {
		...
		CommonResponseModel result = idRegiService.registerId(regiData);
