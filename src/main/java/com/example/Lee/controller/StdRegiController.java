...
public class StdRegiController {

	private final StdRegiService stdRegiService; // 학번 등록 서비스 객체

	@Autowired // 의존성 자동 주입. 스프링이 StdRegiService 타입의 객체를 자동으로 주입
	public StdRegiController(StdRegiService stdRegiService) {
		this.stdRegiService = stdRegiService; // 생성자를 통해 주입받은 서비스 객체를 필드에 할당
	}

	@PostMapping("/PTU/STDregister") // "/PTU/STDregister" 경로로 POST 요청을 받음
	public ResponseEntity<CommonResponseModel> stdRegister(@RequestBody Map<String, String> requestData) {
