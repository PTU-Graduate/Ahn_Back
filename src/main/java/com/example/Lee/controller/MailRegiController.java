...
import com.example.Lee.service.MailRegiService; // 이메일 등록 서비스 클래스

@RestController // 이 클래스가 REST 컨트롤러로 동작함을 스프링에게 알림
public class MailRegiController {

	private final MailRegiService mailRegiService; // 이메일 등록 서비스 객체

	@Autowired // 의존성 자동 주입. 스프링이 MailRegiService 타입의 객체를 자동으로 주입
	public MailRegiController(MailRegiService mailRegiService) {
		this.mailRegiService = mailRegiService; // 생성자를 통해 주입받은 서비스 객체를 필드에 할당
	}

	@PostMapping("/PTU/mailRegister") 
	public ResponseEntity<CommonResponseModel> mailRegister(@RequestBody Map<String, String> requestData) {
		...
		// 서비스를 호출하여 이메일 등록 처리 후 결과를 받음
		CommonResponseModel result = mailRegiService.mailRegister(regiData);
